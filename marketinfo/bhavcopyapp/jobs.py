...
def start():
    ...
    scheduler.add_job(download_manager.download_data, 'cron',hour=18,timezone="Asia/Kolkata",id="download_job")
    scheduler.add_job(data_resetter.reset, 'cron',hour=23,minute=59,second=59,timezone="Asia/Kolkata",id="reset_job")
    # scheduler.add_job(data_resetter.reset, 'interval',seconds=30,timezone="Asia/Kolkata",id="reset_job")
    scheduler.start()